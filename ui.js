...
function renderHallOfFame(){const host=qs('#hofGrid');if(!host)return;const champs=asArray(state.champions?.champions).slice().reverse(),counts={};champs.forEach(c=>counts[c.winner]=(counts[c.winner]||0)+1);const images={'Kristian Radoš|2019/20':'/assets/hof/kristian-rados-master.png','Kristian Radoš|2021/22':'/assets/hof/kristian-rados-master.png','Karlo Medić|2025/26':'/assets/hof/karlo-medic-master.png'};host.innerHTML=champs.map(c=>{const img=images[`${c.winner}|${c.season}`],initials=c.winner.split(/\s+/).map(x=>x[0]).join('').slice(0,2);return`<article class="hof-card ${img?'featured':''}">${counts[c.winner]>1?`<span class="hof-count">${counts[c.winner]}× prvak</span>`:''}<div class="hof-photo">${img?`<img src="${img}" alt="${esc(c.winner)} sa SkyLux trofejem"><span class="trophy-season">${esc(c.season)}</span>`:`<div class="hof-placeholder">${esc(initials)}</div>`}</div><div class="hof-info"><span class="hof-season">${esc(c.season)}</span><h3>${esc(c.winner)}</h3><p>${esc(c.team||'')}</p></div></article>`}).join('')}